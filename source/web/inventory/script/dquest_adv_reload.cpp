...
{
	char query[1001]{};
	sprintf(query, "select count(*) from progress where user_id = '%s' and quest_id = 66 and user_prog < quest_end_prog", user_id);
...
int main(int argc, char *argv[])
{
	...
	if(random_problem_gen(conn, argv[1], 66, 7)) return 1;
	return 0;
}